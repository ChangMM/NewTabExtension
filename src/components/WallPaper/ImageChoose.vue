...
export default {
  ...
  mounted() {
    if (this.m_total == 0) {
      this.$get_image_total().then((data) => {
        this.m_total = data
        this.f_load_images()
      })
    }
      ...
      if (cur < this.m_total) {
        for (let i = 1; i < this.m_per_num; i++){
          if (cur + i  <= this.m_total){
            this.m_images.push('https://cdn.Iizhi.cn/wallpaper/' + (cur + i ) + '.jpg')
            this.m_cur_num ++
          } else {
...
.image-item{
  ...
  .open{
    ...
    background-image: url('https://cdn.Iizhi.cn/icons/eye.png');
    &:hover{
      background-image: url('https://cdn.Iizhi.cn/icons/eye-hover.png');
      .tool-tip{
        display: block;
      }
    }

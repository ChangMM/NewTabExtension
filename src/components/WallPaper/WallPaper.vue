<template>
  <div id="wall-paper">
    <div id="loading" :class="{'disable': this.m_loaded}"></div>
...
export default {
  ...
  data: () => ({
    ...
    m_videoW: 0,
    m_loaded: false,
    m_wallpaper_type: 'image',
    m_video_src: require('@/assets/video_default.mp4'),
    m_image_src: require('@/assets/background_default.jpg')
  }),
  mounted() {
    this.f_init_config()
    Status.$on('wallpaper', (obj) => {
      try {
        ...
        if (this.m_wallpaper_type == 'video') {
          this.m_video_src = 'https://cdn.Iizhi.cn/video/'+ obj.id + '.mp4'
          this.$refs.video.oncanplay = () => {
            this.f_resize_video_size(this.$refs.video, this.$refs.video.videoWidth, this.$refs.video.videoHeight)
          }
        } else {
          this.m_image_src = 'https://cdn.Iizhi.cn/wallpaper/'+ obj.id + '.jpg'
          this.f_init_image()
        }
        chrome.storage.sync.set({'wallpaper': obj}, () => {
          // todo
        })
      } catch (err) {
        // todo
  ...
  methods: {
    f_init_config() {
      try{
        chrome.storage.sync.get(['wallpaper'], (obj) => {
          this.m_wallpaper_type = obj['wallpaper'].type
          if (this.m_wallpaper_type == 'video') {
            this.m_video_src = 'https://cdn.Iizhi.cn/video/'+ obj['wallpaper'].id + '.mp4'
          } else {
            this.m_image_src = 'https://cdn.Iizhi.cn/wallpaper/'+ obj['wallpaper'].id + '.jpg'
          }
        })
      } catch(err) {
        // todo
